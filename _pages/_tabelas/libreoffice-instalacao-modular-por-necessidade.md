...
<table class="evergreen-table">
  ...
  <tbody>
    <tr>
      <td data-label="Ação">Atualizar lista de pacotes</td>
      <td data-label="Comando"><code>sudo apt update</code></td>
      <td data-label="Observações">Sincroniza com todos os repositórios, incluindo <code>trixie/daedalus-backports</code>.</td>
    </tr>
    <tr>
      <td data-label="Ação">Atualizar LibreOffice via backports</td>
      <td data-label="Comando"><code>sudo apt -t daedalus-backports upgrade</code></td>
      <td data-label="Observações">Atualiza **só os pacotes já instalados** (ex: writer, calc) para a versão mais nova do backports. Não instala novos componentes.</td>
    </tr>
  </tbody>
</table>
