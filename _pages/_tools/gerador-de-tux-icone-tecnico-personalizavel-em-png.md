...
<script>
  ...
  break;
        case 'graduation': svg += `<path fill="${hat}" d="M30,30 L70,30 L70,25 L30,25 Z"/><path fill="${hat}" d="M20,25 Q50,35 80,25 Z"/><path fill="${hat}" d="M50,25 Q50,20 45,20 T55,20 Z"/>`; break;
       ...
       case 'santa':
  // Chapéu vermelho: ponta em (50,10), base em y=30
  svg += `<path fill="${hat}" d="M45,30 L55,30 L50,10 Z"/>`;
  ...
  function downloadPNG() {
    ...
    img.onload = () => {
      ...
      const size = 512; // PNG de alta qualidade
      canvas.width = size;
      canvas.height = size;
      ...
      canvas.toBlob(blob => {
        ...
        a.download = 'meu-tux-personalizado.png';
